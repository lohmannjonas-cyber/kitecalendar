import curatedEvents from "../data/curated-events.json";
import { prisma } from "../src/lib/prisma";
import { importEvents, type EventImportRecord } from "./lib/event-importer";
...
const spots = [
  ...
  ["Australia", "Queensland", "Gold Coast", "Broadwater", -27.964, 153.414],
...
function generatedEvents(count = 180): EventImportRecord[] {
...
async function main() {
  const countArg = Number(process.argv.find((arg) => arg.startsWith("--generated="))?.split("=")[1] ?? 180);
  const records = [...(curatedEvents as EventImportRecord[]), ...generatedEvents(countArg)];
  const imported = await importEvents(records);
  console.log(`Seeded ${imported.length} events (${curatedEvents.length} curated, ${countArg} generated starter events).`);
}
